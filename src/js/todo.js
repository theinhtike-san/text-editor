...
const createListUi = (text) => {
  ...
  list.innerHTML = `<div class="flex justify-between mb-5 items-center border-2 border-black p-5 animate__animated animate__fadeInLeft">
  <div class="text-box flex  items-center">
    <input type="checkbox" class="w-5 h-5 input" />
  
    <div class=" ms-2 tasklist-text ">
    ${text}
    </div>
  </div>
  <div class="">
  ...
  taskListDelBtn.addEventListener("click",(delBtnHandler) );

  const tasklistEditBtn = list.querySelector(".tasklist-edit-btn");

  tasklistEditBtn.addEventListener("click",(taskEditBtnHandler));
  
  const textBox = list.querySelector('.text-box');
  textBox.addEventListener("click",(taskDone));
...
const delBtnHandler =(event) => {
  ...
  if(confirm("r u sure to delete")){
    ...
    console.log(list);
    list.children[0].classList.replace("animate__fadeInLeft","animate__backOutRight");
...
const taskEditBtnHandler = (event) => {

  const taskList = event.target.closest(".tasklist");
  const taskListText = taskList.querySelector(".tasklist-text");
  const input = document.createElement("input");
  input.classList.add("border-2","border-black","px-3");
...
const taskDone = (event) => {
  ...
  taskInput.checked == true ? taskListText.classList.add("line-through"): taskListText.classList.remove("line-through");
      countDoneList();
}
...
//     const list = document.createElement("div");
//     list.innerHTML = `
    // <div class="flex justify-between mb-5 items-center border-2 border-black p-5">
